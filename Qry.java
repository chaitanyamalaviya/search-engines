...
public abstract class Qry {
  ...
  public void appendArg(Qry q) throws IllegalArgumentException {
	...
	//  this refers to the operator of the root query
    ...
    //  Iop operators combine inverted lists to give a result inverted list
    ...
    //  QrySop operators and their arguments must be of the QrySop type.
   ...
   */
  // Implemented by subclasses QrySop and QryIop and subclassed operator classes
  ...
  protected boolean docIteratorHasMatchAll (RetrievalModel r) {
    ...
    while (! matchFound) {
      ...
      // Check whether the query argument has a match at all. If not, return false.
      ...
      for (int i=1; i<this.args.size(); i++) {
			...
			if (docid_0 != docid_i) {	// docid_0 can't match.  Try again. Advance docIterator to next docid.
    ...
    for (int i=0; i<this.args.size(); i++) {
      ...
      if (q_i.docIteratorHasMatch (r)) {
        ...
        if ((minDocid > q_iDocid) ||
            (minDocid == Qry.INVALID_DOCID)) {  //Get the smallest docId to match any of the query arguments
        		minDocid = q_iDocid;
        }
      }
    }
