...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    //Split will separate distance of proximity operators
    String[] substrings = operatorNameLowerCase.split("[/]");
    ...
    switch (substrings[0]) {
      ...
      default:  syntaxError ("Unknown query operator " + operatorName);
    }
  ...
  private static Qry parseString (String queryString)
    ...
    //Create operator for default model operator
    
    Qry queryTree = createOperator (substrings[0].trim());
    boolean weightop = false; //Indicates if weight is expected
    if (queryTree instanceof QrySopWand || queryTree instanceof QrySopWsum)
    	weightop = true;
    	
    String op = substrings[0].trim();
    ...
    while (queryString.length() > 0) {
    	  ...
    	  //Extract weight of the argument if weight is expected
    	  if (weightop == true){
    		  substr = queryString.split(" ", 2);
    		  weight = Float.parseFloat(substr[0].trim());
    		  queryString = substr[1].trim();
    	  }
	      ...
	      if (queryString.charAt(0) == '#') {	// Subquery
			  ...
			  qargs = new Qry[1];
			  qargs[0] = parseString (p.getPopped());  //Recurse over popped string = subquery
		      }
	      
	      else {					// Term
			  p = popTerm (queryString); //Pop the first term 
			  qargs = createTerms (p.getPopped()); //Tokenize term and create term operator for each resulting term
	      }
	      ...
	      for (int i=0; i<qargs.length; i++) {
	    	  ...
	    	  //  Handle weights: Add weights to corresponding query operator
	    	  if (weightop == true){
	    		  if (op.toLowerCase().compareTo("#wand") == 0)
	    			  ((QrySopWand) queryTree).addweight(weight);
	    		  else if (op.toLowerCase().compareTo("#wsum") == 0)
