...
public class ScoreList {
    ...
    public int compare(ScoreListEntry s1, ScoreListEntry s2) {
      if (s1.score > s2.score)
    	  ...
    	  if (s1.score < s2.score)
    		  ...
    		  if (s1.externalId.compareTo(s2.externalId)>0)  // The external document id should be a secondary sort key
    			  return 1;
    		  else
  ...
  public boolean containsDocId(int internaldocId){
	  
	  for (int i = 0; i < this.scores.size(); i++){
		  if (this.scores.get(i).docid == internaldocId){
			  return true;
		  }
	  }
	  return false;
  ...
  public int indexOfDoc(int internaldocId){
  ...
  public void removeDoc(int internaldocId){
	  for (int i = 0; i < this.scores.size(); i++){
		  if (this.scores.get(i).docid == internaldocId){
